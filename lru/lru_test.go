...
func TestOnAdded(t *testing.T) {
	...
	onAddedFun := func(key Key, value interface{}) {
		count++
	}
	maxFun := func(c *Cache) bool {
		return count > 20
	...
	// Duplicate the add operation to test that OnEvicted and OnAdded
	// are called when keys are replaced.

	for i := 0; i < 22; i++ {
		lru.Add(fmt.Sprintf("myKey%d", i), 1234)
	}
	if len(evictedKeys) != 24 {
		t.Fatalf("got %d evicted keys; want 2", len(evictedKeys))
	}
	if evictedKeys[0] != Key("myKey0") {
		t.Fatalf("got %v in first evicted key; want %s", evictedKeys[0], "myKey0")
